...
export function TableView({ table, onUpdateTable, onRemoveGuestFromTable, onUpdateGuest }: TableViewProps) {
  ...
  return (
    <Card 
      className={`w-80 h-80 bg-gradient-card border-2 shadow-elegant hover:shadow-lg transition-smooth ${
      ...
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            {table.isHeadTable && <Crown className="w-4 h-4 text-rose-gold" />}
            {editingName ? (
              <Input
                value={newName}
      ...
      <CardContent className="flex-1 p-3">
        <div className="h-full flex flex-col space-y-2 overflow-auto">
          ...
          {table.guests.length === 0 && (
            <div className={`text-center p-4 border-2 border-dashed rounded-lg flex-1 flex items-center justify-center ${
              table.isHeadTable 
